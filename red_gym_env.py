...
from map_pokeball_ids import POKEBALL_NAMES
...
class RedGymEnv(Env):
    ...
    def __init__(
        ...
        self.reset_count = 0
        self.pokeball_reward_points = 0
        self.pokeball_points_scaled = 0
        self.all_runs = []
    ...
    def step(self, action):
        # Initialize new_reward at the start of the method
        new_reward = 0  # Initialize with a starting value
        ...
        # Append current stats without needing to pass pokeball_reward_points
        ...
        self.check_and_print_pokeball_changes(initial_pokeballs, final_pokeballs)

        # Calculate rewards based on the changes in Poké Ball quantities
        pokeball_reward_points = 0  # Reset to 0 before calculation
        for pokeball_id, initial_count in initial_pokeballs.items():
            final_count = final_pokeballs.get(pokeball_id, 0)
        ...
        # Apply scaling to pokeball points before adding to the total reward
        self.pokeball_points_scaled = pokeball_reward_points * 10000

        # Add scaled pokeball points to the total reward only if pokeball is picked up
        if self.pokeball_points_scaled > 0:
            new_reward += self.pokeball_points_scaled
        
        # Continue with the rest of the step method as before
        if self.use_screen_explore:
        ...
        info = {
            ...
            "pokeballs_quantities": final_pokeballs
        }
    ...
    def append_agent_stats(self, action):
        ...
        self.agent_stats.append({
            ...
            'pokeball_reward_points': self.pokeball_points_scaled,
            'pokeballs': pokeballs_quantities
    })
    ...
    def save_and_print_info(self, done, obs_memory):
        if self.print_rewards:
            ...
            prog_string += f' sum: {int(self.total_reward)}'

            # Extract pokeball_reward_points for clarity
            pokeball_points = self.pokeball_reward_points
            prog_string += f', pokeballs points: {pokeball_points:.2f}'
            ...
            if len(self.agent_stats) > 1:  # Ensure there is a previous step to compare with
                ...
                # Summarize gained Poké Balls for display
                gained_summary = sum(gained_pokeballs.values())
                if gained_summary > 0:  # Only add this if there's a positive change
                    prog_string += f',  Gained Pokeballs: {gained_summary}'

                # Optionally, list details of gained Poké Balls
                #gained_details = ', '.join([f"{POKEBALL_NAMES.get(id, 'Unknown')}:+{quantity}" for id, quantity in gained_pokeballs.items() if quantity > 0])
                #if gained_details:  # Add this info only if there's a positive change
                    #prog_string += f' (Details: {gained_details})'
    ...
    def get_game_state_reward(self, print_stats=False):
        ...
        state_scores = {
            ...
            'pokeballs': self.pokeball_points_scaled
    }
    ...
    def monitor_item_bag(self):
        item_bag_start = 0xD31E  # Start right after the total number of items byte
        item_bag_end = 0xD346
        total_items = self.read_m(0xD31D)  # Total number of items in the bag

        # Dictionary to hold item IDs and their quantities
        item_bag_contents = {}

        for i in range(total_items):
            item_address = item_bag_start + i*2  # Each item occupies 2 bytes: ID and quantity
            item_id = self.read_m(item_address)
            quantity_address = item_address + 1
            quantity = self.read_m(quantity_address)

            item_bag_contents[item_id] = quantity
        
        return item_bag_contents

    def monitor_stored_items(self):
        stored_items_start = 0xD53A
        stored_items_end = 0xD59F
        # Assuming a similar structure to the item bag for stored items
        # You would need to adjust based on how stored items are actually managed in memory
        total_stored_items = self.read_m(stored_items_start - 1)  # If the preceding byte contains the count

        stored_items_contents = {}

        for i in range(total_stored_items):
            item_address = stored_items_start + i*2  # Assuming same 2-byte structure: ID and quantity
            item_id = self.read_m(item_address)
            quantity_address = item_address + 1
            quantity = self.read_m(quantity_address)

            stored_items_contents[item_id] = quantity

        return stored_items_contents
    ...
    def check_and_print_pokeball_changes(self, initial_quantities, final_quantities):
        for pokeball_id, initial_quantity in initial_quantities.items():
            final_quantity = final_quantities[pokeball_id]
            if final_quantity > initial_quantity:
                gained = final_quantity - initial_quantity
